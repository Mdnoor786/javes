...
rkfilters.__table__.create(checkfirst=True)


def get_filter(chat_id, keyword):
    try:
        return SESSION.query(rkfilters).get((str(chat_id), keyword))
    except:
        return None
    finally:
        SESSION.close()
...
def get_all_rkfilters(chat_id):
    try:
        return SESSION.query(rkfilters).filter(rkfilters.chat_id == str(chat_id)).all()
    except:
        return None
    finally:
        SESSION.close()
...
def remove_filter(chat_id, keyword):
    if saved_filter := SESSION.query(rkfilters).get((str(chat_id), keyword)):
        SESSION.delete(saved_filter)
        SESSION.commit()


def remove_all_rkfilters(chat_id):
    if saved_filter := SESSION.query(rkfilters).filter(
        rkfilters.chat_id == str(chat_id)
    ):
        saved_filter.delete()
        SESSION.commit()
